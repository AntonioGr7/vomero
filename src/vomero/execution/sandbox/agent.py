...
`corpus.py` source bind-mounted in by path — so it runs on a stock
`python:3.11-slim` image with nothing installed.
...
bind mount (so grep/read stay fast, no per-call round trip), but `llm`, `rlm`,
`answer`, `ask_user`, `ask_parent` and the `todo` surface are RPC stubs: calling
one sends a request back to the host over the control socket and blocks for the
reply. The host runs the real callable (which closes over the engine, meter,
channel, ...) and sends the result back.

Invocation:  python agent.py <socket-path> <corpus-src-path>
...
def _load_corpus(path: str):
    """Load the `Corpus` class from the bind-mounted corpus.py, in isolation.

    Loaded by file path (not `import vomero...`) so the agent needs nothing of
    Vomero on the import path and the package's heavier `__init__` never runs."""
    spec = importlib.util.spec_from_file_location("_vomero_corpus", path)
